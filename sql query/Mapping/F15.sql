...
select A.F1501,
       ...
       A.AddLog_User,
       case
           when A.AddLog_Time is not null then
               cast(concat(Format(A.AddLog_Date, 'yyyy/MM/dd'), ' ', A.AddLog_Time) as datetime)
           else null end                                       as AddLog_Date,
       A.AddLog_IP,
       A.EditLog_User,
       case
