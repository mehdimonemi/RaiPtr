           ...
           -(
               (
                   (CONVERT(FLOAT ,SUBSTRING(F2406 ,1 ,2))- 98)*365*24*60+(
                       CASE 
                            WHEN CONVERT(FLOAT ,SUBSTRING(F2406 ,3 ,2))<=6 THEN (CONVERT(FLOAT ,SUBSTRING(F2406 ,3 ,2))- 1) 
                                *31
               ...
               (
                   (CONVERT(FLOAT ,SUBSTRING(F2408 ,1 ,2))- 98)*365*24*60+(
                       CASE 
                            WHEN CONVERT(FLOAT ,SUBSTRING(F2408 ,3 ,2))<=6 THEN (CONVERT(FLOAT ,SUBSTRING(F2408 ,3 ,2))- 1) 
                                *31
       ...
       LEFT JOIN (
                ...
                FROM   (
                       ...
                       INNER JOIN (
                            ...
                            ON  maxweights.w = allweights.w
                                AND maxweights.F2401 = allweights.F2401
            ) AS finalWeights
...
WHERE  Is_Masdoodi = 0 AND TrainKind = 1 AND (f2404 < @endDate AND f2404 > @startDate)
GROUP BY
       F.F2401
      ,upcode
      ,downcode
