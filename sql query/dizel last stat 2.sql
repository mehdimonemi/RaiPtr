...
FROM (
         ...
         FROM (
                  ...
                  FROM graph.dbo.F22 AS x
                      ...
                      SELECT F4602,
                             ...
                             F1506
                      FROM graph.dbo.F46
                               INNER JOIN graph.dbo.F15 ON F46.F15Rec_ID = F15.Rec_Id
                  ) AS t ON t.F15Rec_ID = x.F15Rec_ID
                      AND (x.f2201 = t.F4610 OR t.F1506 <> x.f2201)
                  ...
                  INNER JOIN graph.dbo.F15 ON s.F15Rec_ID = F15.Rec_Id
     ) AS allTimes
         INNER JOIN (
    SELECT d.F4602,
           MAX(
    ...
    FROM (
             ...
             FROM graph.dbo.F22 AS x
                 ...
                 FROM graph.dbo.F46
                          INNER JOIN graph.dbo.F15 ON F46.F15Rec_ID = F15.Rec_Id
             ) AS t ON t.F15Rec_ID = x.F15Rec_ID AND (x.f2201 = t.F4610 OR t.F1506 <> x.f2201)
             where CAST(CONCAT(RTRIM(LTRIM(F2205)), RTRIM(LTRIM(F2206))) AS FLOAT) < @endDate
               AND CAST(CONCAT(RTRIM(LTRIM(F2205)), RTRIM(LTRIM(F2206))) AS FLOAT) > @startDate
         ) AS d
             INNER JOIN graph.dbo.F22 AS f
                        ON d.F15Rec_ID = f.F15Rec_ID
                            ...
                            AND f.F2206 <= d.F2206
    GROUP BY d.F4602
) AS maxs ON maxs.F4602 = allTimes.F4602 AND maxs.maxEnterTime = allTimes.EnterTime
         INNER JOIN (
    SELECT tso.Tools_No
    FROM graph.dbo.Tools_Status_Op AS tso
        ...
        FROM graph.dbo.F15
                 INNER JOIN graph.dbo.P8 AS p ON p.CODE = F15.f1504
                 INNER JOIN graph.dbo.f24 ON f24.F15Rec_ID = F15.Rec_Id
        GROUP BY f15.Rec_Id,
                 p.kind
    ) AS f ON tso.F15Rec_ID = f.Rec_Id
    WHERE blocks > 4
      AND (f.TrainKind = 1 OR TrainKind = 2)
    GROUP BY tso.Tools_No
) AS dizelList ON allTimes.F4602 = dizelList.Tools_No
